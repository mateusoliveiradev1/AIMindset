    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-primary-dark/95 backdrop-blur-md border-b border-neon-purple/20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          {/* Logo */}
          <Link to="/" className="flex items-center space-x-2 group header-logo">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            {navigation.map((item) => (
              <Link
                key={item.name}
                to={item.href}
                className={`font-montserrat font-medium transition-all duration-300 relative group header-nav-link ${
                  isActive(item.href)
                    ? 'text-lime-green'
          ...
          <div className="flex items-center space-x-4">
            ...
            {shouldShowSearch && (
              <button
                ...
                className="p-2 text-futuristic-gray hover:text-lime-green transition-colors duration-300 header-search-btn"
                aria-label="Buscar"
              >
            ...
            <div className="px-2 pt-2 pb-3 space-y-1 bg-dark-surface/95 backdrop-blur-md rounded-lg mt-2 border border-neon-purple/20">
              {navigation.map((item) => (
                <Link
                  ...
                  className={`block px-3 py-2 rounded-md font-montserrat font-medium transition-all duration-300 header-nav-link ${
                    isActive(item.href)
