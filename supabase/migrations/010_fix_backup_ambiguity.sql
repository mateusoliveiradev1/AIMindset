-- Correção para ambiguidade de backup_id na função backup_all_data

-- 1. Remover função existente
DROP FUNCTION IF EXISTS backup_all_data();

-- 2. Recriar função com aliases corretos para evitar ambiguidade
...
BEGIN
    ...
    DELETE FROM backup_articles WHERE backup_id != (
        SELECT ba.backup_id FROM backup_articles ba ORDER BY ba.backup_created_at DESC LIMIT 1
    );
    DELETE FROM backup_comments WHERE backup_id != (
        SELECT bc.backup_id FROM backup_comments bc ORDER BY bc.backup_created_at DESC LIMIT 1
    );
    DELETE FROM backup_feedbacks WHERE backup_id != (
        SELECT bf.backup_id FROM backup_feedbacks bf ORDER BY bf.backup_created_at DESC LIMIT 1
    );
...
-- Comentário
COMMENT ON FUNCTION backup_all_data() IS 'Função para criar backup completo incluindo logs (últimos 30 dias) - versão corrigida sem ambiguidade';