-- Remover função existente e recriar com a assinatura correta
-- Esta migração resolve o problema de tipo de retorno

-- Primeiro, remover todas as versões existentes da função
DROP FUNCTION IF EXISTS public.get_article_metrics(UUID) CASCADE;
DROP FUNCTION IF EXISTS public.get_article_metrics() CASCADE;
DROP FUNCTION IF EXISTS public.get_article_metrics(article_id_param UUID) CASCADE;

-- Criar função get_article_metrics que funciona corretamente
-- Esta função calcula métricas em tempo real para um artigo específico
...
BEGIN
    ...
    WHERE article_id = target_article_id AND useful = true;

    -- Contar feedbacks negativos
    SELECT COUNT(*) INTO negative_count
    FROM public.feedback
    WHERE article_id = target_article_id AND useful = false;
...
COMMENT ON FUNCTION public.get_article_metrics(UUID) IS 'Calcula métricas em tempo real para um artigo específico incluindo feedbacks, comentários, curtidas e taxa de aprovação';