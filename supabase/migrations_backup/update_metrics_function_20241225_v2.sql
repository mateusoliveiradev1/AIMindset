-- Corrigir função get_article_metrics para usar a estrutura correta da tabela feedbacks
-- Esta migração resolve o problema de tabela e campos incorretos

-- Dropar função existente
DROP FUNCTION IF EXISTS public.get_article_metrics(UUID);

-- Recriar função com a estrutura correta
...
BEGIN
    -- Contar feedbacks positivos (usando tabela 'feedbacks' e campo 'type')
    SELECT COUNT(*) INTO positive_count
    FROM public.feedbacks
    WHERE article_id = target_article_id AND type = 'positive';

    -- Contar feedbacks negativos (usando tabela 'feedbacks' e campo 'type')
